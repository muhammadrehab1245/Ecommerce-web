...
export const OrderList = () => {
    ...
    return (
        <>
            {orders.length === 0 ? (
                <h1 className='text-center mt-10'>Order List are empty</h1>
            ) : (
                orders && orders.map(orderlist => (
                    <div key={orderlist.id} className="mt-8">
                        <div className="flow-root">
                            {orderlist?.cartitem?.map(orderseq => (
                                <li key={orderseq.id} className="flex flex-col md:flex-row py-6">
                                    <div className="h-48 w-full md:h-24 md:w-20 flex-shrink-0 overflow-hidden rounded-md border border-gray-200">
                                        <img
                                    ...
                                    <div className="ml-4 flex flex-1 flex-col">
                                        <div className="flex flex-col md:flex-row mb-4 justify-between text-base font-medium text-blue-50">
                                            <div className="flex flex-col gap-2">
                                                <h3>{orderseq.name}</h3>
                                                <p className="text-blue-50">Total Item Price: ${orderseq.ItemTotal}</p>
                                            </div>
