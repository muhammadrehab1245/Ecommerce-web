...
console.log(data)
  ...
  return (
    <>
    {currentorder && <Navigate to={`/orderstatus/${currentorder.id}`} replace={true} />}
    {cartitem.length === 0 && <Navigate to={`/`} replace={true} />}
    <div className="grid grid-cols-1 px-4 text-white gap-x-6 gap-y-8 lg:grid-cols-4 lg:px-20">
      <div className="text-white col-span-3">
        <form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <button
            type="submit"
            className="flex items-center justify-center rounded-md border border-transparent bg-[#1e40af] px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-indigo-500"
          >
            Checkout
          </button>
        </form>
